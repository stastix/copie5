...
{  
    ...
    {   $user = $userRepository->find($id); 
        ...
        public function UsePts($id,UserRepository $repo,$prix) {  
            ...
            // Calculate the deduction value based on the given price
            $deductionValue = round($prix / 100);
        
            // Check if the user has enough points to deduct
            if ($cartefidelite->getPtsfidelite() >= $deductionValue) {
                $cartefidelite->setPtsfidelite($cartefidelite->getPtsfidelite() - $deductionValue);
                ...
                return $this->redirectToRoute('error_page'); // Change 'error_page' to your actual error route
            }
        }
